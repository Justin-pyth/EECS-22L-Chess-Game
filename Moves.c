...
{
    int cap = 24;
    struct move* moves = malloc((size_t)cap * sizeof(struct move));
    *moveCount = 0;
    ...
    if (nr >= 0 && nr < 8 && board[nr][col] == NULL) {
        moves = appendMove(moves, moveCount, &cap, row, col, nr, col);
        if (row == startRow) {
            int nr2 = row + 2 * dir;
            if (nr2 >= 0 && nr2 < 8 && board[nr2][col] == NULL)
                moves = appendMove(moves, moveCount, &cap, row, col, nr2, col);
        }
    }
    for (int dc = -1; dc <= 1; dc += 2) {
        ...
        if (nr >= 0 && nr < 8 && nc >= 0 && nc < 10 &&
            ...
            moves = appendMove(moves, moveCount, &cap, row, col, nr, nc);
    }
    return moves;
}
    ...
    *moveCount = 0;
    ...
    for (int d = 0; d < 4; d++) {
        ...
        while (r >= 0 && r < 8 && c >= 0 && c < 10) {
            if (board[r][c] == NULL) {
                moves = appendMove(moves, moveCount, &cap, row, col, r, c);
            } else {
    ...
    *moveCount = 0;
    ...
    for (int r = 0; r < 8; r++) {
        for (int c = 0; c < 10; c++) {
            ...
            int cnt = 0;
            struct move* raw = NULL;

            /* Call the appropriate get*Moves function */
            switch (p->piece) {
                case ANT:      raw = getAntMoves     (gs->board, r, c, &cnt); break;
                case BISHOP:   raw = getBishopMoves  (gs->board, r, c, &cnt); break;
                case KNIGHT:   raw = getKnightMoves  (gs->board, r, c, &cnt); break;
                case ROOK:     raw = getRookMoves    (gs->board, r, c, &cnt); break;
                case QUEEN:    raw = getQueenMoves   (gs->board, r, c, &cnt); break;
                case KING:     raw = getKingMoves    (gs->board, r, c, &cnt); break;
                case ANTEATER: raw = getAnteaterMoves(gs->board, r, c, &cnt); break;
            }

            /* Convert each struct move → Move, filter for legality */
            for (int i = 0; i < cnt; i++) {
                int fr, fc, tr, tc;
                decodeMoveCoords(raw[i], &fr, &fc, &tr, &tc);

                int flags = MOVE_NORMAL;

                /* Anteater eating moves get their own flag */
                if (p->piece == ANTEATER &&
                    gs->board[tr][tc] != NULL &&
                    gs->board[tr][tc]->piece == ANT &&
                    gs->board[tr][tc]->color == opp)
                    flags = MOVE_ANTEATING;

                /* Pawn reaching promotion row: emit 5 moves */
                ...
                commitIfLegal(gs->board, raw[i], flags, color, false, moves, moveCount);
            }
            free(raw);

            /* ── Special moves not covered by get*Moves ── */
