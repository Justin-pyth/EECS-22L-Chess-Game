...
void testEdgePositions(struct log* gameLog) {
    ...
    // Black rook h10->h6 (row=7, col=9 -> row=7, col=5)
...
// Test 5: encodeLocation / decodeLocation round-trip
...
int main() {
    ...
    printf("\n");
    printSeparator();
    printf("  ANT CHESS - LOG SYSTEM TESTS\n");
    printSeparator();
    printf("\n");

    testAntOpenings(&gameLog);
    testBackRowPieces(&gameLog);
    testEdgePositions(&gameLog);
    testReplayLog(&gameLog);
    testEncodeDecode();
